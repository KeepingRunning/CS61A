...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    # BEGIN PROBLEM 5
    "*** YOUR CODE HERE ***"
    dif, word = limit + 1, typed_word
    for s in word_list:
        if s == typed_word:
            return s
        diff = diff_function(typed_word, s, limit)
        if diff <= limit and diff < dif:
            dif, word = diff, s
    return word 
    # END PROBLEM 5
...
def furry_fixes(typed, source, limit):
    ...
    if len(typed) == 0:
        return  len(source)
    if source[0] == typed[0]:
        return furry_fixes(source[1:], typed[1:], limit)
    else:
        return furry_fixes(source[1:], typed[1:], limit - 1) + 1 
